...
async def _complete_payment(
    ...
    *,
    ...
    text = (
        ...
        f"<b>Срок:</b> 30 дней\n\n"
        f"🔑 <b>Код активации для приложения Flow:</b>\n"
        f"<code>{code_value}</code>\n\n"
        f"Открой приложение Flow → вкладка «Подписка» → введи этот код.\n\n"
        f"Я начну собирать твой первый бокс к ближайшим месячным. "
        f"Команда /mybox — посмотреть статус."
    )
